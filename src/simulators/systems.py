...
class System(ABC):
    ...
    def get_output(self, states: np.ndarray, noise_flag=True, multi_inp=False) -> np.ndarray:
        ...
        noise = self.m_noise(state=states) if noise_flag else np.zeros_like(states)
...
class Duffing(System):
    ...
    def diff_eq(self, x: list[float], t: float = 0, inp: list[float] = None,
                noise_flag=True) -> np.ndarray:
        if inp is None:
            inp = np.zeros(1)
        x1_dot = x[1] ** 3
        x2_dot = - x[0] + inp[0]
        noise = self.p_noise(state=np.array(x), t=t) if noise_flag else np.zeros(len(x))
...
class VdP(System):
    ...
    def diff_eq(self, x: list[float], t: float = 0, inp: Optional[Union[float, Callable]] = None,
                noise_flag=True) -> np.ndarray:
        if inp is None:
            inp = np.zeros(1)
        x1_dot = x[1]
