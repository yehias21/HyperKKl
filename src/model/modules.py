...
class MainLSTMCell(nn.Module):
    def __init__(self, input_dim, hidden_dim, z_dim):
        super(MainLSTMCell, self).__init__()
        self.weight_ih = nn.Linear(z_dim, 4*hidden_dim, bias=False)
        self.weight_hh = nn.Linear(z_dim, 4*hidden_dim, bias=False)
        self.bias = nn.Linear(z_dim, 4*hidden_dim, bias=False)
        self.W_ih = nn.Linear(input_dim, 4*hidden_dim)
        self.W_hh = nn.Linear(hidden_dim, 4*hidden_dim)
        
    def forward(self, z,inputs , state):
        h,c = state
        weight_ih = self.weight_ih(z)
        weight_hh = self.weight_hh(z)
        bias = self.bias(z)
        ...
        ifgo = HadamardLinear(inputs, weight_ih, bias) +  HadamardLinear(h_, weight_hh, bias)  
        i, f, g, o = torch.chunk(ifgo, 4, -1)
        f = torch.sigmoid(f)
        g = torch.tanh(g)
        o = torch.sigmoid(o)
...
class MainRNN(nn.Module):
    def __init__(self,
        ...
        if cell_type =='rnn':
            ...
            self.cell = MainLSTMCell(main_input_dim, main_dim, z_dim)
